...
from tasks.task_controller import judge_submit_async, judge_verify_submit_async, judge_diffcult_submit_async
...
class FinishJudgeTask(APIView):
    ...
    def post(self, request, task_id, format=None):
        ...
        if all_selected and self.task.status != Task.STATUS_FINISHED:
            ...
            elif self.task.typ == Task.TYPE_JUDGE_DIFFCULT:
                judge_diffcult_submit_async(task_id)
            return Response({'task_id': task_id, 'status': self.task.status})
